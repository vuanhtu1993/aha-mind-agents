...
export class GeminiRateLimiterService {
  ...
  private checkAndResetMinuteInterval() {
    ...
    if (now - this.lastMinuteResetTime >= 60000) {
      ...
      this.logger.debug('🔄 Đã reset quota của phút hiện tại.');
    }
  }
  ...
  private async processRequest<T>(estimatedTokens: number, operation: () => Promise<T>): Promise<T> {
    ...
    if (willExceedRPM || willExceedTPM) {
      ...
      this.logger.warn(`⏳ Đạt giới hạn phút (RPM: ${this.requestsThisMinute}/${this.RPM_LIMIT}, TPM: ${this.tokensThisMinute}/${this.TPM_LIMIT}). Đang sleep ${Math.ceil(waitTimeMs / 1000)} giây...`);
    ...
    this.logger.debug(`🚀 Thực thi request (RPM: ${this.requestsThisMinute}/${this.RPM_LIMIT}, Tokens ~ ${estimatedTokens})`);
